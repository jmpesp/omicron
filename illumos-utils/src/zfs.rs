...
impl EnsureDatasetVolumeError {
    ...
    pub fn from_raw_zvol_open(name: String, e: rustix::io::Errno) -> Self {
        if e == rustix::io::Errno::INPROGRESS {
            ...
            EnsureDatasetVolumeError {
                name,
                err: EnsureDatasetVolumeErrorInner::Open(e),
            }
        }
    }
}
...
impl DeleteDatasetVolumeError {
    ...
    pub fn from_raw_zvol_open(name: String, e: rustix::io::Errno) -> Self {
        if e == rustix::io::Errno::INPROGRESS {
            ...
            DeleteDatasetVolumeError {
                name,
                err: DeleteDatasetVolumeErrorInner::Open(e),
            }
        }
    }
}
...
impl Zfs {
    ...
    pub async fn ensure_dataset_volume(
        ...
        if params.raw {
            // Open the raw zvol: if EINPROGRESS is seen, then it's not done
            // initializing yet. We can't use it until it's fully done, so
            // return the `NotReady` variant to signal upstack that it should
            // poll.
    ...
    pub async fn delete_dataset_volume(
        ...
        if params.raw {
            // Open the raw zvol: if EINPROGRESS is seen, then it's not done
            // initializing yet. We can't delete it until it's fully done, so
            // return the `NotReady` variant to signal upstack that it should
            // poll.
            let path = format!("/dev/zvol/rdsk/{}", params.name);
