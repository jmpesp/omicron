  ...
  new_vmm_local_storage_records
    AS (
      ...
      INTO
        vmm_local_storage
      VALUES
        (gen_random_uuid(), $9, $10, $11, $12, $13), (gen_random_uuid(), $14, $15, $16, $17, $18)
      RETURNING
        *
    ),
  update_local_storage_records
    AS (
      UPDATE
  ...
  insert_valid
    AS (
      ...
      WHERE
        ...
        AND NOT (EXISTS(SELECT 1 FROM banned_sleds WHERE sled_id = $19))
        AND (
            EXISTS(SELECT 1 FROM required_sleds WHERE sled_id = $20)
        ...
        AND (
            (
              ...
              FROM
                crucible_dataset
                JOIN local_storage_dataset ON
                    crucible_dataset.pool_id = local_storage_dataset.pool_id
                JOIN new_vmm_local_storage_records ON
            ...
            AND (
              ...
              < (
                  ...
                  - (SELECT control_plane_storage_buffer FROM zpool WHERE id = $27)
                )
            AND (
                SELECT
                  sled.sled_policy = 'in_service'
