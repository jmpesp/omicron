...
//! handles the following snapshot replacement request state transitions:
//!
//! ```text
//!         Requested   <--
//!                       |
...
// snapshot replacement start saga: input parameters
...
declare_saga_actions! {
    ...
    REPLACE_SNAPSHOT_IN_VOLUME -> "unused_2" {
        + ssrs_replace_snapshot_in_volume
        - ssrs_replace_snapshot_in_volume_undo
    }
    UPDATE_REQUEST_RECORD -> "unused_4" {
        + ssrs_update_request_record
    }
}

// snapshot replacement start saga: definition

#[derive(Debug)]
pub(crate) struct SagaSnapshotReplacementStart;
impl NexusSaga for SagaSnapshotReplacementStart {
    const NAME: &'static str = "snapshot-replacement-start";
    type Params = Params;

    fn register_actions(registry: &mut ActionRegistry) {
        snapshot_replacement_start_register_actions(registry);
    }
...
// snapshot replacement start saga: action implementations

async fn ssrs_set_saga_id(
    ...
    // avoids Nexus allocating a bunch of replacement snapshots only to unwind
    // all but one.
    osagactx
        .datastore()
        .set_snapshot_replacement_allocating(&opctx, params.request.id, saga_id)
        .await
...
async fn ssrs_set_saga_id_undo(
    sagactx: NexusActionContext,
) -> Result<(), anyhow::Error> {
    let osagactx = sagactx.user_data();
    let params = sagactx.saga_params::<Params>()?;
    ...
    osagactx
        .datastore()
        .undo_set_snapshot_replacement_allocating(
            &opctx,
            params.request.id,
            saga_id,
        )
...
async fn ssrs_get_alloc_region_params(
    sagactx: NexusActionContext,
...
async fn ssrs_alloc_new_region(
    sagactx: NexusActionContext,
...
async fn ssrs_alloc_new_region_undo(
    sagactx: NexusActionContext,
) -> Result<(), anyhow::Error> {
    let osagactx = sagactx.user_data();
    ...
    // It should be guaranteed that if ssrs_alloc_new_region succeeded then it
...
async fn ssrs_new_region_ensure(
    sagactx: NexusActionContext,
) -> Result<
...
async fn ssrs_new_region_ensure_undo(
    sagactx: NexusActionContext,
) -> Result<(), anyhow::Error> {
    let log = sagactx.user_data().log();
    let osagactx = sagactx.user_data();

    warn!(log, "ssrs_new_region_ensure_undo: Deleting crucible regions");
...
async fn ssrs_get_old_snapshot_volume_id(
    sagactx: NexusActionContext,
...
async fn ssrs_replace_snapshot_in_volume(
    sagactx: NexusActionContext,
) -> Result<(), ActionError> {
    let log = sagactx.user_data().log();
    let osagactx = sagactx.user_data();
...
async fn ssrs_replace_snapshot_in_volume_undo(
    sagactx: NexusActionContext,
) -> Result<(), anyhow::Error> {
...
async fn ssrs_update_request_record(
    sagactx: NexusActionContext,
) -> Result<(), ActionError> {
    let params = sagactx.saga_params::<Params>()?;
    let osagactx = sagactx.user_data();
...
pub(crate) mod test {
    use crate::{
        ...
        app::sagas::snapshot_replacement_start::Params,
        app::sagas::snapshot_replacement_start::SagaSnapshotReplacementStart,
    ...
    async fn test_snapshot_replacement_start_saga(
        cptestctx: &ControlPlaneTestContext,
    ) {
        ...
        // Manually insert the snapshot replacement request
        let request =
            SnapshotReplacement::for_region_snapshot(&region_snapshot);

        datastore
            .insert_snapshot_replacement_request(&opctx, request.clone())
            .await
            .unwrap();

        // Run the snapshot replacement start saga
        let dag = create_saga_dag::<SagaSnapshotReplacementStart>(Params {
    ...
    pub(crate) async fn verify_clean_slate(
        ...
        // destination volume
        assert_eq!(region_allocations(&datastore).await, 6);
        assert_snapshot_replacement_request_untouched(
    ...
    async fn assert_snapshot_replacement_request_untouched(
        cptestctx: &ControlPlaneTestContext,
        datastore: &DataStore,
        request: &SnapshotReplacement,
    ) {
    ...
    async fn test_actions_succeed_idempotently(
        ...
        let dag =
            create_saga_dag::<SagaSnapshotReplacementStart>(params).unwrap();
