...
//! Snapshot replacement is distinct from region replacement: replacing parts of
//! a volume's read-only parent (and all the layers under it) is easier because
//! this does _not_ incur a live repair or reconciliation. Each part of a
//! read-only region set contains the same data that will never be modified.
//!
//! A snapshot replacement request starts off in the "Requested" state, just
//! like a region replacement request. A background task will search for
//! snapshot replacement requests in this state and trigger the "snapshot
//! replacement start" saga. This will allocate a new region to replace the
//! requested one, and modify the snapshot VCR accordingly. If any disks are
...
//! replaced snapshot (which could be gone if it is expunged). It is this saga's
//! responsibility to update all Volumes that reference the snapshot being
//! replaced, and send a replacement request to any Upstairs that were
//! constructed.
//!
...
//! reference. This is done as a "snapshot replacement step", and once all those
//! are done, the snapshot replacement request can be "completed".
//!
//! Snapshot replacement steps need to be written into the database and have an
...
// snapshot replacement step saga: input parameters
...
// snapshot replacement step saga: actions

declare_saga_actions! {
    snapshot_replacement_step;
    SET_SAGA_ID -> "unused_1" {
        + srss_set_saga_id
        - srss_set_saga_id_undo
    }
    CREATE_REPLACE_PARAMS -> "replace_params" {
        + srss_create_replace_params
    }
    CREATE_FAKE_VOLUME -> "unused_2" {
        + ssrs_create_fake_volume
        - ssrs_create_fake_volume_undo
    }
    REPLACE_SNAPSHOT_IN_VOLUME -> "unused_3" {
        + srss_replace_snapshot_in_volume
        - srss_replace_snapshot_in_volume_undo
    }
    NOTIFY_UPSTAIRS -> "unused_4" {
        + srss_notify_upstairs
    }
    UPDATE_REQUEST_RECORD -> "unused_5" {
        + srss_update_request_record
    }
}

// snapshot replacement step saga: definition

#[derive(Debug)]
pub(crate) struct SagaSnapshotReplacementStep;
impl NexusSaga for SagaSnapshotReplacementStep {
    const NAME: &'static str = "snapshot-replacement-step";
    type Params = Params;

    fn register_actions(registry: &mut ActionRegistry) {
        snapshot_replacement_step_register_actions(registry);
    }
...
// snapshot replacement step saga: action implementations

async fn srss_set_saga_id(
...
async fn srss_set_saga_id_undo(
    sagactx: NexusActionContext,
) -> Result<(), anyhow::Error> {
    let osagactx = sagactx.user_data();
    let params = sagactx.saga_params::<Params>()?;
    ...
    osagactx
        .datastore()
        .undo_set_snapshot_replacement_step_running(
            &opctx,
            params.request.id,
            saga_id,
        )
...
async fn srss_create_replace_params(
    sagactx: NexusActionContext,
    ...
    // look up snapshot replace request by id

    let snapshot_replace_request = osagactx
        .datastore()
        .get_snapshot_replacement_request_by_id(
            &opctx,
    ...
    let region_snapshot = osagactx
        .datastore()
        .region_snapshot_get(
            snapshot_replace_request.old_dataset_id,
            snapshot_replace_request.old_region_id,
            snapshot_replace_request.old_snapshot_id,
        )
        .await
        .map_err(ActionError::action_failed)?;
    ...
    let Some(new_region_id) = snapshot_replace_request.new_region_id else {
        return Err(ActionError::action_failed(format!(
            "request {} does not have a new_region_id!",
            snapshot_replace_request.id,
        )));
    };
...
async fn ssrs_create_fake_volume(
    sagactx: NexusActionContext,
) -> Result<(), ActionError> {
    let osagactx = sagactx.user_data();
...
async fn ssrs_create_fake_volume_undo(
    sagactx: NexusActionContext,
) -> Result<(), anyhow::Error> {
    let osagactx = sagactx.user_data();
...
async fn srss_replace_snapshot_in_volume(
    sagactx: NexusActionContext,
) -> Result<(), ActionError> {
    let osagactx = sagactx.user_data();
    let params = sagactx.saga_params::<Params>()?;
    ...
    osagactx
        ...
        .volume_replace_snapshot(
            ...
            /* target */
            replace_params.old_snapshot_address,
            /* replacement */
...
async fn srss_replace_snapshot_in_volume_undo(
    sagactx: NexusActionContext,
) -> Result<(), anyhow::Error> {
    let osagactx = sagactx.user_data();
    let params = sagactx.saga_params::<Params>()?;
...
async fn srss_notify_upstairs(
    ...
    info!(
        ...
        state {state}";
        "snapshot replacement volume id" => %params.request.id,
        "disk id" => ?disk.id(),
...
async fn srss_update_request_record(
    sagactx: NexusActionContext,
) -> Result<(), ActionError> {
    let params = sagactx.saga_params::<Params>()?;
    let osagactx = sagactx.user_data();
