...
impl super::Nexus {
    async fn user_data_export_for_resource(
            ...
            match client.volume_status(&volume_id.to_string()).await {
                Err(e) => {
                    ...
                    let delete_record = match e {
                        crucible_pantry_client::Error::ErrorResponse(_) => {
                        ...
                        crucible_pantry_client::Error::CommunicationError(_) => {
                        ...
                        crucible_pantry_client::Error::InvalidRequest(_)
                        | crucible_pantry_client::Error::InvalidUpgrade(_)
                        | crucible_pantry_client::Error::ResponseBodyError(_)
                        | crucible_pantry_client::Error::InvalidResponsePayload(
                            _,
                            _,
                        )
                        | crucible_pantry_client::Error::UnexpectedResponse(_)
                        | crucible_pantry_client::Error::PreHookError(_)
                        | crucible_pantry_client::Error::PostHookError(_) => {
        ...
        tokio::spawn({
            let client = self.reqwest012_client.clone();
            let log =
